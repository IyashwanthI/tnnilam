...
@Component({
  selector: "app-isd-form",
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  template: `
    <div class="tn-page">
      <h1 class="page-title">INVOLVING SUBDIVISION</h1>
      ...
      @if (submitted) {
        ...
        <div class="form-card">
          <form [formGroup]="form" (ngSubmit)="onSubmit()" novalidate>
            ...
            <div class="form-row two-col">
              <div class="form-group">
                <label>Sub Division No. <span class="req">*</span></label>
                <input
                  type="text"
                  class="form-ctrl"
                  ...
                  placeholder="e.g. 1A"
                />
                @if (f["subdivNo"].invalid && f["subdivNo"].touched) {
                  <span class="err">Sub Division No. is required.</span>
                }
              </div>
              <div class="form-group">
...
export class IsdFormComponent implements OnInit {
  districts: District[] = [];
  taluks: Taluk[] = [];
  villages: Village[] = [];
  loading = false;
  ...
  onSubmit(): void {
    ...
    setTimeout(() => {
      this.loading = false;
      this.submitted = true;
      this.applId = "ISD" + Date.now().toString().slice(-8);
    }, 800);
  }
