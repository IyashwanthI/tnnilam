...
@Component({
  selector: "app-nisd-form",
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  template: `
    <div class="tn-page">
      <h1 class="page-title">NOT INVOLVING SUBDIVISION</h1>
...
export class NisdFormComponent implements OnInit {
  districts: District[] = [];
  taluks: Taluk[] = [];
  villages: Village[] = [];
  loading = false;
  ...
  onSubmit(): void {
    ...
    setTimeout(() => {
      this.loading = false;
      this.submitted = true;
      this.applId = "NISD" + Date.now().toString().slice(-8);
    }, 800);
  }
